...
class BaseAgenticVLMReasoner(ABC):
    ...
    def _build_system_prompt(
        ...
        return (
            ...
            "1. Queries are built from words representing NOUNS (objects, target entities, visual appearance) and VERBS/ACTIONS (motion, behavior, direction, actions).\n"
            "2. When calling `encode_and_search_vector_store`, encode ONLY NOUNS and static visual entity descriptors (e.g. 'red car', 'person in black shirt', 'white pickup truck') into `query_text` to retrieve candidate tracks.\n"
            "   - DO NOT include verbs, actions, motions, or dynamic behaviors (e.g. 'turning left', 'running', 'walking past', 'parking', 'driving away') in the vector search `query_text`.\n"
            "   - Crop/image vector store embeddings encode static appearance (nouns) effectively, but do not encode dynamic actions.\n"
            "3. Retrieve candidate target tracks from the vector store using the noun-focused query.\n"
            "4. VERIFY VERBS & ACTIONS VISUALLY: After retrieving candidates, use `inspect_visual_candidate`, `get_entire_frame`, or `get_temporal_context` to visually inspect frames and verify whether the described verbs, actions, and behaviors actually occurred.\n\n"
            f"## Available Tools\n{tool_decls_json}\n\n"
            "## Tool Usage Rules\n"
            "1. `encode_and_search_vector_store` retrieves candidates using NOUN-ONLY encodings. "
            "It does NOT verify correctness or actions. Always follow up with visual inspection to verify verbs/actions.\n"
            "2. `inspect_visual_candidate` extracts a frame crop/context and returns it to you. "
            "YOU must analyze the attached image to verify both noun appearance and described verbs/actions. "
            ...
            "Use this for relationship queries (e.g., 'bus followed by car') or action context verification.\n"
            "5. `get_entire_frame` extracts the full, uncropped video frame for a given camera ID and timestamp. "
            "Use this when full scene context, trajectory, or motion verification is needed.\n\n"
            "## Relationship & Action Queries\n"
            "For queries involving actions or multiple objects:\n"
            "1. Extract primary noun entities to query the vector store.\n"
            "2. Inspect candidate frames to visually verify if the verbs/actions (e.g., turning, walking, stopping) match the query.\n"
            "3. Use `get_temporal_context` for multi-object spatial-temporal relationships.\n"
            "4. Only confirm candidates if both the visual target (noun) AND the described action (verb) are visually verified.\n"
            ...
            "</final_answer>\n"
            "```\n"
            "- `confidence` must be grounded in your visual analysis of inspected crops and verified actions.\n"
            "- `explanation` must reference specific visual evidence (nouns and verified verbs/actions) you observed.\n"
